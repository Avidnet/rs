...
class Storage {
  ...
  */
  ...
  /**
   * fetch retrieves value for given location with 100m precision.
   */
  async fetch (lat, lng) {
    const query = `SELECT * FROM points WHERE ST_DWithin(location, 'SRID=4326;POINT(${lat} ${lng})'::geography, 100)`
